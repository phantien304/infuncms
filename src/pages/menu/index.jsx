/**
 * pages/menu/index.jsx — DANH SÁCH menu (REST chuẩn).
 * -----------------------------------------------------------
 * Convert từ mt219 resources/js/cms/components/menu/index.vue.
 * Mirror pages/category/index.jsx. Cột đặc thù: Position (top/footer).
 *
 *   GET    /menu            (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /menu/{id}       (xoá mềm 1)
 *   PATCH  /menu/{id}/restore (khôi phục 1)
 *   POST   /menu/bulk       ({ action:'delete'|'restore', ids:[] })
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ...
                    : api.patch(`/menu/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/menu/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
                        ...
                        <tbody>
                            {list.map((item) => (
                                <tr key={item.id} className="post-item-group">
                                    ...
                                    <td>{item.id}</td>
                                    <td>
                                        <Link to={`/menu/${item.id}`} style={{ color: '#0f74a8' }}>
                                            {item.title}
                                        </Link>
                                    </td>
                                    <td>{item.position}</td>
