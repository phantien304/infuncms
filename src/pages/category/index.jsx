/**
 * pages/category/index.jsx — DANH SÁCH category (REST chuẩn).
 * -----------------------------------------------------------
 * Khác bản trước (envelope /list): bản này gọi REST qua `api`:
 *   GET    /category            (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /category/{id}       (xoá mềm 1)
 *   PATCH  /category/{id}/restore (khôi phục 1)
 *   POST   /category/bulk       ({ action:'delete'|'restore', ids:[] })
...
export default function CategoryIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/category/${item.id}`)
                    : api.patch(`/category/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/category/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('CategoryList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
