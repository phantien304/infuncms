/**
 * pages/contact/index.jsx — DANH SÁCH contact (mailbox liên hệ khách hàng
 * gửi từ storefront). Convert từ mt219 cms/components/contact/index.vue.
 * Mirror pages/filter/index.jsx nhưng KHÔNG có nút Add (khách hàng tự gửi
 * form, admin không tạo contact thủ công) — dùng FormSearch showAdd={false}.
 * -----------------------------------------------------------
 *   GET    /contact              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /contact/{id}         (xoá mềm 1)
 *   PATCH  /contact/{id}/restore (khôi phục 1)
 *   POST   /contact/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function ContactIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/contact/${item.id}`)
                    : api.patch(`/contact/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/contact/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('ContactList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        showAdd={false}
                        ...
                        <table className="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    >
                                        {t('Phone')} {sortIcon('phone')}
                                    </th>
                                    <th
                                        style={{ color: '#1e91cf', cursor: 'pointer' }}
                                        onClick={() => onSort('service')}
                                    >
                                        {t('Service')} {sortIcon('service')}
                                    </th>
                                    <th>{t('Content')}</th>
                                    <th
                                        className="width-150"
