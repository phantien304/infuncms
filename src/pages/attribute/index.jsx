/**
 * pages/attribute/index.jsx — DANH SÁCH attribute (REST chuẩn, convert từ
 * mt219 cms/components/attribute/index.vue). Mirror 1:1 pages/filter/index.jsx.
 * -----------------------------------------------------------
 *   GET    /attribute              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /attribute/{id}         (xoá mềm 1)
 *   PATCH  /attribute/{id}/restore (khôi phục 1)
 *   POST   /attribute/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function AttributeIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/attribute/${item.id}`)
                    : api.patch(`/attribute/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/attribute/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('AttributeList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/attribute/add"
                        objAction={objAction}
