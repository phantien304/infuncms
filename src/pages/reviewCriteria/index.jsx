/**
 * pages/reviewCriteria/index.jsx — DANH SÁCH tiêu chí chấm điểm review
 * (review_criteria — vd "Chất lượng sản phẩm", "Đóng gói"). Tính năng MỚI
 * hoàn toàn trong infun (không có ở mt219). Mirror pages/filter/index.jsx.
 * -----------------------------------------------------------
 *   GET    /review-criteria              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /review-criteria/{id}         (xoá mềm 1)
 *   PATCH  /review-criteria/{id}/restore (khôi phục 1)
 *   POST   /review-criteria/bulk         ({ action:'delete'|'restore', ids:[] })
 * -----------------------------------------------------------
 */
...
export default function ReviewCriteriaIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/review-criteria/${item.id}`)
                    : api.patch(`/review-criteria/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/review-criteria/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('ReviewCriteriaList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/review-criteria/add"
                        objAction={objAction}
                    ...
                    <table className="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th className="width-100">{t('Code')}</th>
                                <th className="width-100">{t('SortOrder')}</th>
                                <th className="width-100">{t('Required')}</th>
                        ...
                        <tbody>
                            {list.map((item) => (
                                <tr key={item.id} className="post-item-group">
                                    ...
                                    <td>{item.is_required ? t('Yes') : t('No')}</td>
