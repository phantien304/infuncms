/**
 * pages/banner/index.jsx — DANH SÁCH banner (REST chuẩn, convert từ mt219
 * cms/components/banner/index.vue).
 * -----------------------------------------------------------
 *   GET    /banner              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /banner/{id}         (xoá mềm 1)
 *   PATCH  /banner/{id}/restore (khôi phục 1)
 *   POST   /banner/bulk         ({ action:'delete'|'restore', ids:[] })
 * Response list: { data:[...], meta:{ total, ... } }. item.page là mảng
 * (BannerData::fromModel tách sẵn từ cột page dạng "home,category" trong DB).
 * -----------------------------------------------------------
 */
...
export default function BannerIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/banner/${item.id}`)
                    : api.patch(`/banner/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/banner/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('BannerList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/banner/add"
                        ...
                        <tbody>
                            {list.map((item) => (
                                <tr key={item.id} className="post-item-group">
                                    ...
                                    <td>{item.type === 'image' ? t('Image') : t('Slider')}</td>
                                    <td>
                                        {(item.page || []).map((code) => (
                                            <span
                                                key={code}
