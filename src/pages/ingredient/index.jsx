/**
 * pages/ingredient/index.jsx — DANH SÁCH ingredient (thành phần mỹ phẩm/INCI).
 * Convert từ mt219 cms/components/ingredient/index.vue. Mirror
 * pages/manufacturer/index.jsx (KHÔNG i18n, bảng phẳng).
 * -----------------------------------------------------------
 *   GET    /ingredient              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /ingredient/{id}         (xoá mềm 1)
 *   PATCH  /ingredient/{id}/restore (khôi phục 1)
 *   POST   /ingredient/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function IngredientIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/ingredient/${item.id}`)
                    : api.patch(`/ingredient/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/ingredient/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('IngredientList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
