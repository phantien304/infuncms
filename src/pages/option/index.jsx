/**
 * pages/option/index.jsx — DANH SÁCH option (REST chuẩn, convert từ mt219
 * cms/components/option/index.vue). Mirror pages/filter/index.jsx, thêm cột
 * Type (để phân biệt nhanh custom field / variant widget) và NameDisplay
 * (giống list gốc mt219, sort theo option_description.name_display).
 * -----------------------------------------------------------
 *   GET    /option              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /option/{id}         (xoá mềm 1)
 *   PATCH  /option/{id}/restore (khôi phục 1)
 *   POST   /option/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function OptionIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/option/${item.id}`)
                    : api.patch(`/option/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/option/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('OptionList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/option/add"
