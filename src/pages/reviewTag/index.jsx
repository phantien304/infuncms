/**
 * pages/reviewTag/index.jsx — DANH SÁCH nhãn review (review_tag — vd
 * "Chất lượng tốt", "Giao hàng nhanh"). Tính năng MỚI hoàn toàn trong infun.
 * Mirror pages/filter/index.jsx.
 * -----------------------------------------------------------
 *   GET    /review-tag              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /review-tag/{id}         (xoá mềm 1)
 *   PATCH  /review-tag/{id}/restore (khôi phục 1)
 *   POST   /review-tag/bulk         ({ action:'delete'|'restore', ids:[] })
 * -----------------------------------------------------------
 */
...
export default function ReviewTagIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/review-tag/${item.id}`)
                    : api.patch(`/review-tag/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/review-tag/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('ReviewTagList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/review-tag/add"
                        objAction={objAction}
                    ...
                    <table className="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th className="width-150">{t('Code')}</th>
                                <th className="width-150">{t('UsageCount')}</th>
