/**
 * pages/manufacturer/index.jsx — DANH SÁCH manufacturer (REST chuẩn, convert
 * từ mt219 cms/components/manufacturer/index.vue). Mirror pages/filter/index.jsx
 * nhưng KHÔNG có i18n (manufacturer chỉ 1 bảng, không có bảng dịch) — thêm cột
 * Image (thumbnail) thay cho cột SortOrder như list gốc mt219.
 * -----------------------------------------------------------
 *   GET    /manufacturer              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /manufacturer/{id}         (xoá mềm 1)
 *   PATCH  /manufacturer/{id}/restore (khôi phục 1)
 *   POST   /manufacturer/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function ManufacturerIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/manufacturer/${item.id}`)
                    : api.patch(`/manufacturer/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/manufacturer/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('ManufacturerList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/manufacturer/add"
                        objAction={objAction}
