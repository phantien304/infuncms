/**
 * pages/information/index.jsx — DANH SÁCH information (trang nội dung tĩnh:
 * hướng dẫn mua hàng, chính sách, điều khoản...). Convert từ mt219
 * cms/components/information/index.vue. Mirror pages/filter/index.jsx.
 * -----------------------------------------------------------
 *   GET    /information              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /information/{id}         (xoá mềm 1)
 *   PATCH  /information/{id}/restore (khôi phục 1)
 *   POST   /information/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function InformationIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/information/${item.id}`)
                    : api.patch(`/information/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/information/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('InformationList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/information/add"
                        objAction={objAction}
