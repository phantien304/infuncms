/**
 * pages/filter/index.jsx — DANH SÁCH filter (REST chuẩn, convert từ mt219
 * cms/components/filter/index.vue).
 * -----------------------------------------------------------
 *   GET    /filter              (list, params: keyword/sort/order/deleted_at/page/per_page)
 *   DELETE /filter/{id}         (xoá mềm 1)
 *   PATCH  /filter/{id}/restore (khôi phục 1)
 *   POST   /filter/bulk         ({ action:'delete'|'restore', ids:[] })
...
export default function FilterIndex() {
        ...
        confirm(t(willDelete ? 'DoYouWantToDeleteItem' : 'DoYouWantToRecover'))
            .then(() => {
                ...
                const req = willDelete
                    ? api.delete(`/filter/${item.id}`)
                    : api.patch(`/filter/${item.id}/restore`);
                req
                    .then(() => {
    ...
    const bulkAction = (action) => {
        ...
        confirm(t('DoYouWantUpdateSelected'))
            .then(() => {
                const inst = loading.open();
                api
                    .post('/filter/bulk', { action: act, ids: action.selected })
                    .then(() => {
                        success(t('Successful'));
    ...
    return (
        <Wrapper title={t('FilterList')} sapo="">
            {loader ? (
                <>
                    <FormSearch
                        addPath="/filter/add"
